...
#'  - Normal: `norm` - Parameters are `mean` and `sd`.
#'  - Lognormal: `lnorm` - Parameters are `meanlog` and `sdlog`.
#'  - Gamma: `gamma` - Shape, Rate, Scale
#'  - LogGamma: `lgamma` - Shapelog, Ratelog
#'  - Pareto: `pareto` - Shape and Scale
#'  - Weibull: `weibull` - Shape and Scale
#'  - Generalized Beta: `genbeta` - Shape1, Shape2, Shape3, Rate, Scale
#'
...
simulate_claims <- function(n_claims = 1000,
  ...
  checkmate::assert(
    ...
    checkmate::check_choice(loss_distribution, choices = c("normal",
                                                           "norm",
                                                           "lognorm",
                                                           "lnorm",
                                                           "lognormal",
...
# summ <- 0
#
# for (i in seq_along(payments)) {
#
#   x <- (log(payments[i]) - 7.5)^2
#   summ <- summ + x
#
# }
#
...
#
#
# get_distribution_function <- function(
#     n_claims,
#     dist = c("normal", "lognormal", "gamma", "loggamma", "pareto", "weibull", "genbeta"),
#     ...) {
#
#   dist <- match.arg(dist)
#
#   out <- switch(dist,
#                 "normal" = stats::rnorm(n = n_claims, 5000, sd = )
#                 "lognormal" = stats::rlnorm(n = n_claims))
#
# }
#
# normal_mu <- 5500
# normal_sd <- 4.75
# lognormal_mu <- 7.5
# lognormal_sd <- 1.5 # sqrt(sum((log(payments) - 7.5) ^ 2)/(n_claims-1))
# gamma_shape <-
